...
\begin{itemize}[nosep,leftmargin=1.5em]
    ...
    \item \textbf{Tolerances are unit-independent.} Convergence checks
          based on $\|\Delta y\|$ or $\|g_y\|_\infty$ are invariant under
          a change of physical units, which is essential for a structural
          model whose variables come from different design domains.
\end{itemize}
...
the global bounds $y^L \leq y_k + \Delta y \leq y^U$. A first attempt at
the SLP subproblem is the pure LP
\begin{align}
    \min_{\Delta y} \quad
        & g_{y,k}^{\!\top} \Delta y, \\
    \text{subject to} \quad
...
direction $\Delta y$ from the LP, the trial point is
\begin{align}
    y_{\mathrm{trial}}(\alpha) = y_k + \alpha\, \Delta y,
\end{align}
with $\alpha \in (0, 1]$. A backtracking line search starts from
$\alpha = 1$ and successively shrinks $\alpha \leftarrow \beta\,\alpha$
with $\beta \in (0, 1)$ (typically $\beta = 0.5$) until an acceptance
condition is met. The line search is called \emph{inexact} because it
does not minimize $\Phi$ exactly along the direction; it only enforces a
sufficient decrease.
...
how far in scaled coordinates the LP is allowed to step. In trust-region
theory \citep[Ch.~6]{Conn2000},\citep[Ch.~4]{NocedalWright2006},
...
\toprule
\multicolumn{2}{l}{\textbf{Algorithm 1: Self-written SLP iteration}}\\
\midrule
1.  & Evaluate $f(x_k)$ and $c(x_k)$. \\
2.  & Estimate $g_x$, $J_x$ by bound-aware finite differences. \\
3.  & Transform to scaled space: $g_y = D g_x,\; J_y = J_x D$. \\
4.  & Build and solve the slack-relaxed LP for $(\Delta y, s)$. \\
5.  & Compute the predicted merit decrease
      $\Delta\Phi_{\mathrm{pred}}(\alpha)$. \\
6.  & Backtrack on $\alpha = 1, \beta, \beta^2, \dots$ until either
      Armijo merit decrease or feasibility-progress acceptance holds. \\
7.  & If accepted, update $x_{k+1}, y_{k+1}, f_{k+1}, c_{k+1}$ and the
      history. Otherwise, mark the iteration rejected. \\
8.  & Compute $\rho_k$ and update $\delta$:
      enlarge if $\rho > 0.75$, shrink if $\rho < 0.25$. \\
9.  & If slacks were active or feasibility stalled, increase $M$. \\
10. & Check primary and secondary convergence criteria. If a criterion
      holds, terminate with success; if $\delta < \delta_{\min}$,
...
$\varepsilon_{x}$                 & $10^{-4}$ & step-size tolerance (scaled)\\
