...
class OptimizationLogger:
    ...
    def __init__(
        ...
        os.makedirs(save_folder, exist_ok=True)                                     # Creates the folder "Optimization_Logs".
        self.txt_path = os.path.join(save_folder, f"optimization_log_{ts}.txt") 
        self.csv_path = os.path.join(save_folder, f"objective_history_{ts}.csv") 
        self._write_header(x0, bounds, method)                                      # Writes Header
        ...
        with open(self.csv_path, "w", encoding="utf-8") as f: # This line writes the header to the objective history file.
            ...
            f.write("\n\n")
            f.write(f"iteration,eval_index,objective,{x_headers},g_max,v_agg,c_val\n")
    ...
    def log_evaluation(self, x, fun, v_agg=None,g_max=None,p_value=None,c_value = None): # When called, it will log the evaluation of the objective function and design variables, and write it inside the Optimization_Log folder files.
